...
# -----------------------------------------------------------------------------
# html2docx import shim:
# Try in this order:
#   1) add_html_to_document(doc, html)
#   2) HtmlToDocx().add_html_to_document(doc, html)
#   3) html2docx(...) -> Document, then deep-copy body into target doc
#      (supports various function signatures requiring/allowing 'title')
# -----------------------------------------------------------------------------
# -----------------------------------------------------------------------------
# html2docx import shim
# -----------------------------------------------------------------------------
...
_add_html_impl_name = None

def add_html_wrapper(doc_like, html: str) -> None:
    raise RuntimeError(f"{LOG_PREFIX} html2docx wrapper not initialized")

try:
    from html2docx import add_html_to_document as _add_html_to_document  # type: ignore
    _add_html_impl_name = "add_html_to_document"
    def add_html_wrapper(doc_like, html: str) -> None:
        _add_html_to_document(doc_like, html)

except Exception:
    try:
        from html2docx import HtmlToDocx  # type: ignore
        _add_html_impl_name = "HtmlToDocx.add_html_to_document"
        _converter = HtmlToDocx()
        def add_html_wrapper(doc_like, html: str) -> None:
            _converter.add_html_to_document(doc_like, html)

    except Exception:
        try:
            from html2docx import html2docx as _html2docx_func  # type: ignore
            _add_html_impl_name = "html2docx (copy body)"

            def add_html_wrapper(doc_like, html: str) -> None:
                tmp = None
                try:
                    # try straight call
                    tmp = _html2docx_func(html)
                except TypeError:
                    try:
                        tmp = _html2docx_func(html, "Quill Fragment")
                    except TypeError:
                        tmp = _html2docx_func("Quill Fragment", html)

                # normalize tmp into a Document
                if isinstance(tmp, Document):
                    tmp_doc = tmp
                elif isinstance(tmp, (bytes, bytearray)):
                    tmp_doc = Document(io.BytesIO(tmp))
                elif hasattr(tmp, "read"):  # file-like
                    tmp_doc = Document(tmp)
                elif isinstance(tmp, str) and os.path.isfile(tmp):
                    tmp_doc = Document(tmp)
                else:
                    raise TypeError(f"{LOG_PREFIX} html2docx() returned unsupported type {type(tmp)}")

                # copy body elements into our subdoc
                target_body = doc_like.element.body
                for child in list(tmp_doc.element.body):
                    target_body.append(deepcopy(child))

        except Exception as e:
            raise ImportError(
                f"{LOG_PREFIX} Could not import any html2docx API. "
                f"Install/upgrade html2docx with:\n"
                f"    pip install --upgrade html2docx\n"
                f"Original error: {e}"
            )

logger.debug(f"{LOG_PREFIX} html2docx backend in use: {_add_html_impl_name}")
...
class WordSubmissionHandler:
    ...
    def _build_subdoc_from_html(self, html: str):
        ...
        add_html_wrapper(sub, html)  # <-- shimmed function handles all versions
